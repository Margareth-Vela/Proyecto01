...
reiniciar_tmr0 macro ; Reinicio de Timer0
    ...
    bcf ((INTCON) and 07Fh), 2 ; Limpiar bandera de interrupción por overflow
    endm
...
isr:
    btfsc ((INTCON) and 07Fh), 2 ; Testear la bandera de interrupción del TMR0
...
incrementar_sem1:
    ...
    btfss STATUS, 2 ;si ((STATUS) and 07Fh), 2 no está encendido entonces incrementa
    incf tiempo_sem_1, F
    btfss STATUS, 2 ;y regresa a la subrutina de modo 1
    goto $+3
    movlw 10 ;si ((STATUS) and 07Fh), 2 está encendido entonces llegó al valor máximo y
...
decrementar_sem1:
    ...
    btfss STATUS, 2 ;si ((STATUS) and 07Fh), 2 no está encendido entonces decrementa
    decf tiempo_sem_1, F
    btfss STATUS, 2 ;y regresa a la subrutina de modo 1
    goto $+3
    movlw 20 ;si ((STATUS) and 07Fh), 2 está encendida entonces llegó al valor mínimo y
...
decrementar_sem2:
    movf tiempo_sem_2, W
    sublw 10
    btfss STATUS, 2 ;si ((STATUS) and 07Fh), 2 es 0 entonces decrementa
    decf tiempo_sem_2, F
    btfss STATUS, 2 ;si ((STATUS) and 07Fh), 2 es 1 entonces mueve el valor de 20 a la variable
    goto $+3
    movlw 20
    movwf tiempo_sem_2
    return
...
config_int:
    ...
    Banksel PORTA
    bcf ((PIR1) and 07Fh), 0 ; Se limpia la bandera del TMR1
    return
...
config_tmr0:
    Banksel TRISA
    bcf ((OPTION_REG) and 07Fh), 5 ; TMR0 funciona con reloj interno
    bcf ((OPTION_REG) and 07Fh), 3 ; Prescaler asignado a Timer0
    bsf ((OPTION_REG) and 07Fh), 2
    bsf ((OPTION_REG) and 07Fh), 1
    bsf ((OPTION_REG) and 07Fh), 0 ; Prescaler de 1:256
...
config_tmr1:
    ...
    bcf ((T1CON) and 07Fh), 1 ; Selección del reloj interno
